...
file_path = r"c:\Users\User\Desktop\MOS_Burger\mos_product_frontend\most_product_frontend\src\pages\Menu.jsx"
...
const Menu = () => {
  ...
  const mapToppingIcon = (keyword) => {
    const map = {
      'cheese': '🧀', 'bacon': '🥓', 'jalapeno': '🌶️', 'egg': '🍳', 
      'sauce': '🥫', 'spicy': '🔥', 'ice': '🧊', 'lemon': '🍋', 
      'boba': '🧋', 'chocolate': '🍫', 'strawberry': '🍓', 'sprinkles': '✨'
    };
    for (let key in map) {
  ...
  const calculateTotalPrice = () => {
    ...
    const toppingsTotal = Object.entries(selectedToppings).reduce((acc, [topId, count]) => {
      const toppingInfo = toppings.find(t => t.id === parseInt(topId));
      return acc + (toppingInfo ? parseFloat(toppingInfo.extra_price) * count : 0);
    }, 0);
  ...
  const handleAddToCart = () => {
    ...
    const finalId = hasToppings ? `${modalItem.id}-${new Date().getTime()}` : modalItem.id;
  ...
  if (searchQuery.trim() !== "") {
    displayContent = categories.map(cat => {
      const matchedItems = cat.menus.filter(item => 
    ...
    const selected = categories.find(cat => cat.id === selectedCategory.id) || selectedCategory;
  ...
  return (
    <div className="bg-light min-vh-100 pb-5">
          ...
          <Col md={3} className="mb-4">
            ...
            >
              ...
              <div className="d-flex flex-column gap-2">
                {categories.map((cat) => {
                  const isSelected = selectedCategory?.id === cat.id && !searchQuery.trim();
                  return (
                    <div
                      key={cat.id}
                    ...
                    <div key={categoryGroup.id} className="mb-5">
                        ...
                        {categoryGroup.menus.map((item) => {
                            ...
                            <Card 
                                ...
                                onClick={() => handleShowModal({ ...item, image: itemImage }, categoryGroup.id)}
                            >
                                {/* Price Tag Circle */}
                                <div 
                                ...
                                <img src={itemImage} alt={item.name} className="w-100 h-100" style={{ objectFit: "cover" }} />
                                </div>

                                {/* Bottom Details Section */}
                                <Card.Body className="d-flex flex-column p-4 bg-white">
                                <div className="d-flex justify-content-between align-items-start mb-2">
                                    <Card.Title className="fw-bold fs-5 mb-0 text-dark">{item.name}</Card.Title>
                                </div>
                                <Card.Text className="text-muted small mb-4 flex-grow-1">{item.description}</Card.Text>
                                <div className="mt-auto d-flex justify-content-end">
                                    <Button
                                      variant="danger"
                                      className="rounded-circle p-0 d-flex align-items-center justify-content-center shadow-sm hover-zoom"
                                      style={{ width: "45px", height: "45px" }}
                                      onClick={(e) => { e.stopPropagation(); handleShowModal({ ...item, image: itemImage }, categoryGroup.id); }}
                                    >
      ...
      {/* Item & Toppings Modal */}
      <Modal show={showModal} onHide={handleCloseModal} size="xl" centered contentClassName="border-0 overflow-hidden" style={{ borderRadius: "20px" }}>
        {modalItem && (
          <Row className="g-0 h-100">
            {/* Left Box: Image with mixed blend mode / Background */}
            <Col md={5} className="position-relative d-flex align-items-center justify-content-center" style={{ backgroundColor: "#ffcd38", minHeight: "400px" }}>
              <Button
                variant="light"
                className="position-absolute rounded-circle shadow-sm d-md-none"
                onClick={handleCloseModal}
                style={{ top: "15px", left: "15px", width: "40px", height: "40px", zIndex: 10, padding: 0 }}
              >
                <FiX size={20} />
              </Button>
              <img 
                src={modalItem.image} 
                alt={modalItem.name} 
                className="img-fluid position-relative z-1 p-4" 
                style={{ mixBlendMode: "multiply", filter: "drop-shadow(0px 10px 15px rgba(0,0,0,0.3))", maxHeight: "100%", width: "100%", objectFit: "contain" }} 
              />
            </Col>

            {/* Right Box: Toppings & Details */}
            <Col md={7} className="d-flex flex-column bg-white">
              <div className="p-4 p-md-5 d-flex flex-column h-100 position-relative">
                <Button
                  variant="light"
                  className="position-absolute d-none d-md-flex align-items-center justify-content-center rounded-circle shadow-sm"
                  onClick={handleCloseModal}
                  style={{ top: "20px", right: "20px", width: "40px", height: "40px", padding: 0 }}
                >
                  <FiX size={20} />
                </Button>

                <h2 className="fw-bold mb-1 text-dark" style={{ fontFamily: "inherit"}}>{modalItem.name}</h2>
                <h4 className="text-danger fw-bold mb-3">${parseFloat(modalItem.base_price || modalItem.price || 0).toFixed(2)}</h4>
                <p className="text-muted mb-4">{modalItem.description}</p>

                <h6 className="fw-bold mb-3 text-dark text-uppercase tracking-widest">Customize & Add Toppings</h6>
                
                <div className="flex-grow-1 overflow-auto pe-2 mb-4" style={{ maxHeight: "350px" }}>
                  {toppings.filter(t => t.category === modalItem.category_id).length === 0 ? (
                    <div className="text-muted p-3 bg-light rounded text-center">No extra toppings available for this item.</div>
                  ) : (
                    toppings.filter(t => t.category === modalItem.category_id).map(topping => (
                      <div key={topping.id} className="d-flex align-items-center justify-content-between p-3 mb-2 bg-light rounded-3 transition-all">
                        <div className="d-flex align-items-center gap-3">
                          <div className="fs-3 bg-white rounded-circle shadow-sm d-flex align-items-center justify-content-center" style={{ width: "50px", height: "50px" }}>
                            {mapToppingIcon(topping.image_url)}
                          </div>
                          <div>
                            <h6 className="fw-semibold mb-0 text-dark">{topping.name}</h6>
                            <small className="text-danger fw-bold">+${parseFloat(topping.extra_price).toFixed(2)}</small>
                          </div>
                        </div>
                        <div className="d-flex align-items-center bg-white rounded-pill px-2 py-1 shadow-sm border">
                          <Button variant="link" className="p-1 px-2 text-dark text-decoration-none" onClick={() => handleToppingChange(topping.id, -1)} disabled={!selectedToppings[topping.id]}>
                            <FiMinus size={16} />
                          </Button>
                          <span className="fw-bold px-2" style={{ minWidth: "25px", textAlign: "center" }}>{selectedToppings[topping.id] || 0}</span>
                          <Button variant="link" className="p-1 px-2 text-dark text-decoration-none" onClick={() => handleToppingChange(topping.id, 1)}>
                            <FiPlus size={16} />
                          </Button>
                        </div>
                      </div>
                ...
                <div className="mt-auto pt-3 border-top d-flex justify-content-between align-items-center">
                  <div>
                    <small className="text-muted d-block text-uppercase fw-bold">Total Price</small>
                    <h3 className="fw-bold text-dark mb-0">${calculateTotalPrice().toFixed(2)}</h3>
                  </div>
                  <Button variant="danger" size="lg" className="rounded-pill px-5 d-flex align-items-center gap-2 fw-bold shadow hover-zoom" onClick={handleAddToCart} style={{ borderRadius: "2rem" }}>
                    <FiShoppingCart /> Add {calculateTotalPrice() > parseFloat(modalItem.base_price || modalItem.price || 0) ? "Custom" : ""} Order
                  </Button>
                </div>
              </div>
            </Col>
...
print("Menu.jsx updated successfully!")
